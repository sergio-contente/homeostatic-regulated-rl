#!/usr/bin/env python3
"""
Experiment B: Adjusted a/b parameters.

Uses max{0, 3.0 - 0.5*E} instead of max{0, 1.0 - 0.5*E}.
Social cost vanishes at stock=6.0 instead of stock=2.0, so beta matters
across all tested stock levels (1.0, 2.5, 5.0).
...
STOCKS = [1.0, 2.5, 5.0]
TIMESTEPS = 50_000
LOG_DIR = "experiments/adjusted_ab"
SEED = 1
...
def run(beta, stock):
    ...
    cmd = [
        ...
        "--scarcity_mode", "adjusted_ab",
        "--log_dir", log_dir,
        "--seed", str(SEED),
    ]
    print(f"\n{'='*60}")
    print(f"  [adjusted_ab] beta={beta}, stock={stock}")
    print(f"{'='*60}\n")
...
def main():
    ...
    print(f"Adjusted a/b experiment: {len(experiments)} runs")
