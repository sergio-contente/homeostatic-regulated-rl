#!/usr/bin/env python3
"""
Experiment A: Soft scarcity function.

Uses exp(-E) instead of max{0, a-bE}. Social cost never fully vanishes,
just decays smoothly with abundance. This should make beta visible at all stock levels.
...
STOCKS = [1.0, 2.5, 5.0]
TIMESTEPS = 50_000
LOG_DIR = "experiments/soft_scarcity"
SEED = 1
...
def run(beta, stock):
    ...
    print(f"\n{'='*60}")
    print(f"  [soft] beta={beta}, stock={stock}")
    print(f"{'='*60}\n")
...
def main():
    ...
    print(f"Soft scarcity experiment: {len(experiments)} runs")
