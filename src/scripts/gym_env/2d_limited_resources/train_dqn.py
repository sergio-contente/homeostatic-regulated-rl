...
log_dir = os.path.join('runs', f'DQN_2DLimitedResources_{drive_type}_{current_time}')
...
def process_observation(observation):
    ...
    position = observation['position'].astype(np.float32)
...
for i_episode in range(num_episodes):
    ...
    # Log positions (x, y)
    writer.add_scalar('States/position_x', position[0], total_steps)
    writer.add_scalar('States/position_y', position[1], total_steps)
    episode_reward = 0
    episode_loss = 0
    for t in count():
        total_steps += 1
