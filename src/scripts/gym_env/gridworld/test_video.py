...
# Function to capture pygame display and save frame
...
for i_episode in range(num_test_episodes):
    ...
    while not done and step < 500:  # Limit of 500 steps
        ...
        # Capture frame after the environment is updated
        frames.append(capture_pygame_screen())
    ...
    # Save the captured frames as an MP4 video
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    video_path = os.path.join(videos_dir, f"episode_{i_episode+1}_{timestamp}.mp4")
    
    # Process frames for video
    # Note: frames are in RGB format, need to convert to BGR for OpenCV
    frames_bgr = [frame.transpose(1, 0, 2) for frame in frames]
    frames_bgr = [cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) for frame in frames_bgr]
